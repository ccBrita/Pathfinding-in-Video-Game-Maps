...
class Dijkstra(Search):
    ...
    def search(self, start, goal):
        ...
        while self.OPEN:
            ...
            if n == goal:
                return n.get_cost(), expand
            for n1 in self.map.successors(n):
                expand += 1
                n1.set_cost(n1.get_g())
                if n1.state_hash() not in self.CLOSED:
                    heapq.heappush(self.OPEN, n1)
                    self.CLOSED[n1.state_hash()] = n1
        
                if (n1.state_hash() in self.CLOSED) and (n1.get_g() < self.CLOSED[n1.state_hash()].get_cost()):
                    self.OPEN.remove(n1)
                    heapq.heapify(self.OPEN)
                    heapq.heappush(self.OPEN, n1)
                    self.CLOSED[n1.state_hash()].set_g(n1.get_g())
                    self.CLOSED[n1.state_hash()].set_cost(n1.get_cost())
        return -1, 0
...
class AStar(Search):
    ...
    def h_value(self, state, goal):
        detX = goal.get_x() - state.get_x()
        detY = goal.get_y() - state.get_y()
        if detX < 0:
            detX = -detX
        if detY < 0:
            detY = -detY
        h = 0.5*(max(detX,detY) + 0.5 * min(detX, detY))
        return h
...
h
        ...
        while self.OPEN:
            ...
            for n1 in self.map.successors(n):
                n1.set_h(self.h_value(n1, goal))
                n1.set_cost(n1.get_g()+ n1.get_h())
                expand += 1
                if n1.state_hash() in self.CLOSED:
                    continue
            
                if n1 in self.OPEN:
                    if self.OPEN[self.OPEN.index(n1)].get_g() > n1.get_g():
                        self.OPEN.remove(n1)
                        heapq.heapify(self.OPEN)
                        heapq.heappush(self.OPEN, n1)
                else:
